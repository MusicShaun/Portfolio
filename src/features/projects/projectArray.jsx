
const projectArray = [
  {
    id: 0,
    img: 'CLOTHING-STORE-2_pzzi2q', 
    alt:'MERN clothing store',
    title: 'Clothing store - WIP', 
    text: 'Super fast, custom login, built on nextJS, MongoDb & Node', 
    color: '#F2BC8D', 
    url: 'https://tiny-macaron-3acacd.netlify.app/',
    sandbox: 'https://codesandbox.io/p/github/MusicShaun/eCommerce-frontend/main?file=%2FREADME.md&workspace=%257B%2522activeFilepath%2522%253A%2522%252FREADME.md%2522%252C%2522openFiles%2522%253A%255B%2522%252FREADME.md%2522%255D%252C%2522sidebarPanel%2522%253A%2522EXPLORER%2522%252C%2522gitSidebarPanel%2522%253A%2522COMMIT%2522%252C%2522spaces%2522%253A%257B%2522clgfsk072000x3b6l3f3g19vp%2522%253A%257B%2522key%2522%253A%2522clgfsk072000x3b6l3f3g19vp%2522%252C%2522name%2522%253A%2522Default%2522%252C%2522devtools%2522%253A%255B%257B%2522type%2522%253A%2522PREVIEW%2522%252C%2522taskId%2522%253A%2522dev%2522%252C%2522port%2522%253A3000%252C%2522key%2522%253A%2522clgga55xj00co3b6lpb9xcmmr%2522%252C%2522isMinimized%2522%253Afalse%257D%252C%257B%2522type%2522%253A%2522TASK_LOG%2522%252C%2522taskId%2522%253A%2522dev%2522%252C%2522key%2522%253A%2522clgga54w6009g3b6le68mdu1e%2522%252C%2522isMinimized%2522%253Afalse%257D%255D%257D%257D%252C%2522currentSpace%2522%253A%2522clgfsk072000x3b6l3f3g19vp%2522%252C%2522spacesOrder%2522%253A%255B%2522clgfsk072000x3b6l3f3g19vp%2522%255D%252C%2522hideCodeEditor%2522%253Afalse%257D'

  },
  {
    id: 1, img: 'MERN-BLOG_hoac7w', alt: 'mern stack blog',
    title: 'MERN stack', 
    text: 'Very nice RTK, user authentication, tidy Node server', 
    color: '#F2BC8D', 
    url: 'https://mern-project-main.herokuapp.com/',
    sandbox: 'https://codesandbox.io/p/github/MusicShaun/full-stack-mern-app/main?file=%2FREADME.md&workspace=%257B%2522activeFilepath%2522%253A%2522%252F.gitignore%2522%252C%2522openFiles%2522%253A%255B%2522%252FREADME.md%2522%255D%252C%2522sidebarPanel%2522%253A%2522EXPLORER%2522%252C%2522gitSidebarPanel%2522%253A%2522COMMIT%2522%252C%2522spaces%2522%253A%257B%2522clggaouz9011s3b6lp3maarpf%2522%253A%257B%2522devtools%2522%253A%255B%257B%2522type%2522%253A%2522TASK_LOG%2522%252C%2522taskId%2522%253A%2522heroku-postbuild%2522%252C%2522key%2522%253A%2522clggaouz8011r3b6lykxpjlvt%2522%252C%2522isMinimized%2522%253Afalse%257D%255D%252C%2522key%2522%253A%2522clggaouz9011s3b6lp3maarpf%2522%252C%2522name%2522%253A%2522heroku-postbuild%2522%257D%257D%252C%2522currentSpace%2522%253A%2522clggaouz9011s3b6lp3maarpf%2522%252C%2522spacesOrder%2522%253A%255B%2522clggaouz9011s3b6lp3maarpf%2522%255D%252C%2522hideCodeEditor%2522%253Afalse%257D'

  },
  {
    id: 2, img: 'SWEARDLE_uyepqg', alt: 'wordle with profanity',
    title: 'M15+', 
    text: 'Wordle with profanity? ', 
    color: '#98D0DF', 
    url: 'https://musical-froyo-937a39.netlify.app/',
    sandbox: 'https://codesandbox.io/p/github/MusicShaun/wordle-imitation/master?file=%2FREADME.md&workspace=%257B%2522activeFilepath%2522%253A%2522%252FREADME.md%2522%252C%2522openFiles%2522%253A%255B%2522%252FREADME.md%2522%255D%252C%2522sidebarPanel%2522%253A%2522EXPLORER%2522%252C%2522gitSidebarPanel%2522%253A%2522COMMIT%2522%252C%2522spaces%2522%253A%257B%2522clggb0b7t000x3b6lc5lzmujj%2522%253A%257B%2522key%2522%253A%2522clggb0b7t000x3b6lc5lzmujj%2522%252C%2522name%2522%253A%2522Default%2522%252C%2522devtools%2522%253A%255B%257B%2522key%2522%253A%2522clggb0b7v000y3b6lr8qtuu9g%2522%252C%2522type%2522%253A%2522PROJECT_SETUP%2522%252C%2522isMinimized%2522%253Afalse%257D%255D%257D%257D%252C%2522currentSpace%2522%253A%2522clggb0b7t000x3b6lc5lzmujj%2522%252C%2522spacesOrder%2522%253A%255B%2522clggb0b7t000x3b6lc5lzmujj%2522%255D%252C%2522hideCodeEditor%2522%253Afalse%257D'
  },
  {
    id: 3, img: 'WEBSITE-TEMPLATE_jyyypt', alt: 'beautiful website design',
    title: 'Website Template', 
    text: 'Custom template with NextJS and redux toolkit', 
    color: '#F2BC8D', 
    url: 'https://63b5019e64a3400009783017--prismatic-stroopwafel-c65a19.netlify.app/',
    sandbox: 'https://codesandbox.io/p/github/MusicShaun/Website-design-navigation-via-columns/main?file=%2FREADME.md&workspace=%257B%2522activeFilepath%2522%253A%2522%252FREADME.md%2522%252C%2522openFiles%2522%253A%255B%2522%252FREADME.md%2522%255D%252C%2522sidebarPanel%2522%253A%2522EXPLORER%2522%252C%2522gitSidebarPanel%2522%253A%2522COMMIT%2522%252C%2522spaces%2522%253A%257B%2522clggavrm6000x3b6la0isrkq1%2522%253A%257B%2522key%2522%253A%2522clggavrm6000x3b6la0isrkq1%2522%252C%2522name%2522%253A%2522Default%2522%252C%2522devtools%2522%253A%255B%257B%2522key%2522%253A%2522clggavrm6000y3b6l3bovdbfl%2522%252C%2522type%2522%253A%2522PROJECT_SETUP%2522%252C%2522isMinimized%2522%253Atrue%257D%252C%257B%2522type%2522%253A%2522PREVIEW%2522%252C%2522taskId%2522%253A%2522dev%2522%252C%2522port%2522%253A3000%252C%2522key%2522%253A%2522clggawbu6006o3b6l0ap2fc6x%2522%252C%2522isMinimized%2522%253Afalse%257D%252C%257B%2522type%2522%253A%2522TASK_LOG%2522%252C%2522taskId%2522%253A%2522dev%2522%252C%2522key%2522%253A%2522clggawavs003t3b6lvlwa0o9v%2522%252C%2522isMinimized%2522%253Afalse%257D%255D%257D%257D%252C%2522currentSpace%2522%253A%2522clggavrm6000x3b6la0isrkq1%2522%252C%2522spacesOrder%2522%253A%255B%2522clggavrm6000x3b6la0isrkq1%2522%255D%252C%2522hideCodeEditor%2522%253Afalse%257D'

  }, 
  {
    id: 4, img: 'MORAL-QUIZ_qvp1wc', alt: 'quiz app',
    title: 'Quiz', 
    text: 'State. Many, many states.', 
    color: '#F4A86A', 
    url: 'https://elegant-pony-0acf58.netlify.app/'
}, 
  {
    id: 5, img: 'TODO-LIST_mfaqhy', alt: 'todo list',
    title: 'Todo List', 
    text: 'Simple array manipulation with local storage.', 
    color: '#90ABD1', 
    url: 'https://marvelous-kheer-2d615e.netlify.app/'
} ,
  {
    id: 6, img: 'MASTERING-SITE_ah1aag', alt: 'mastering website',
    title: 'Mastering Website', 
    text: 'Among the first websites I\'d created, this is my current Mastering business.', 
    color: '#FFE07B', 
    url: 'https://www.digital-bath-studio.com/'
}, 
  {
    id: 7, img: 'BANANA-SHOP_vlr6tn', alt: 'banana store',
    title: 'Banana Store', 
    text: 'React store with a mock product api and Stripe payment.', 
    color: '#F4A86A', 
    url: 'https://playful-pudding-43fa23.netlify.app'
}
,

]; 

export default projectArray